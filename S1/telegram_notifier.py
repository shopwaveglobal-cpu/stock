...
# S1 폴더의 slack_notifier는 Real_Time_Monitor.py에서 직접 import하여 사용
# telegram_notifier.py에서는 텔레그램 전송만 담당
...
def send_daily_report(alerts: List[dict], total_stocks: int, recipients: List[str] = None, system_label: str = "S1"):
    """
    ...
    message = f"📊 <b>[{system_label}] 일일 트레이딩 리포트</b>\n"
    message += f"🕐 {now}\n"
    message += f"───────────────\n\n"
    ...
    # Slack 메시지 전송은 Trading_Signal_System_S1.py에서 직접 호출하므로 여기서는 생략
    # (이미 Trading_Signal_System_S1.py에서 send_slack_daily_report을 호출하고 있음)
...
def send_realtime_alert(alert_type: str, stock_name: str, ticker: str, 
                       ...
                       sell_prices: dict = None, system_label: str = "S1",
