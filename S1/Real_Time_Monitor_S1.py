...
def get_enhanced_price_data(ticker: str, token: str) -> Optional[Dict]:
    ...
    try:
        ...
        try:
            import math
            volume_raw = latest.get('trde_qty', 0)
            if volume_raw is None:
                data['volume'] = 0
            else:
                volume_str = str(volume_raw).replace(",", "")
                if volume_str and volume_str.lower() not in ['nan', 'none', '']:
                    volume_float = float(volume_str)
                    if not math.isnan(volume_float):
                        data['volume'] = int(volume_float)
                    else:
                        data['volume'] = 0
                else:
                    data['volume'] = 0
...
def check_simplified_alert(
    ...
    """
    간단한 알람 조건 체크 및 텔레그램/슬랙 전송 (Excel 기반)
...
def run_simplified_monitoring_cycle():
    ...
    try:
        ...
        for idx, row in df_summary.iterrows():
            ...
            buy1 = safe_float_local(buy1_raw)
            buy2 = safe_float_local(buy2_raw)
            buy3 = safe_float_local(buy3_raw)
            ...
            # 이격도 출력 (None 처리)
            dist1_str = f"{dist1:.1f}%" if dist1 is not None else "N/A"
            dist2_str = f"{dist2:.1f}%" if dist2 is not None else "N/A"
            dist3_str = f"{dist3:.1f}%" if dist3 is not None else "N/A"
...
# ── 락 파일: 중복 실행 방지 + 야간 대기 중 강제 종료 감지 ──────────────────
LOCK_FILE = "realtime_monitor.lock"


def acquire_lock() -> bool:
    """중복 실행 방지용 락 파일 획득. 이미 실행 중이면 False 반환."""
    ...
    if os.path.exists(LOCK_FILE):
        try:
            ...
            if pid_is_running(existing_pid):
                logger.error(f"✗ 이미 실행 중인 모니터가 있습니다 (PID: {existing_pid}). 종료합니다.")
...
def sleep_with_lock_check(seconds: float):
    """지정 시간 대기하되 60초마다 락 보유 여부 확인. 락 잃으면 즉시 종료."""
