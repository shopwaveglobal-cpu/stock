...
MONITORING_INTERVAL = 600   # 10분 간격 (초)
...
def get_ticker_data(markets: List[str]) -> List[Dict]:
    ...
    """
    try:
        # 업비트 API는 최대 100개 마켓까지 한 번에 조회 가능
        ...
        logger.info(f"✓ 티커 데이터 {len(ticker_data)}개 조회 완료")
        return ticker_data
...
def send_dropdown_alert(analysis_result: Dict) -> bool:
    ...
    message = f"🚨 <b>코인 저점 시그널 알림</b> 🚨\n"
    message += f"──────────────────\n\n"
    message += f"📊 전일대비 15% 이상 하락: <b>{drop_count}개</b>\n"
...
def run_monitoring_cycle() -> bool:
    ...
    try:
        ...
        # 2. 티커 데이터 조회
        ticker_data = get_ticker_data(krw_markets)
        if not ticker_data:
        ...
        if TELEGRAM_AVAILABLE:
            send_error_alert(str(e), "upbit_alert_monitor")
        return False
...
def main():
    ...
    메인 실행 함수
    """
    logger.info("업비트 시장 하락 감시 시스템 시작")
    ...
    logger.info(f"모니터링 간격: {MONITORING_INTERVAL}초")
